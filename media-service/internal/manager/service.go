...
func (m *mediaService) GetMedia(input *domain.GetMediaInput) (*domain.Media, error) {
  ...
  if !found {
    return nil, apperror.NewError(apperror.ErrTypeNotFoundContent, &apperror.LogMessage{
      Level: apperror.LogLevelWarn,
      Message: fmt.Sprintf("media with name '%s', section '%s', id '%s' not found",
