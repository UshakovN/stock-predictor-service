...
func ReadRequest(r *http.Request, req any) error {
  ...
  case http.MethodPost:
    ...
    if len(content) == 0 {
      return apperror.NewError(apperror.ErrTypeBodyNotFound, nil)
    }

    if err = json.Unmarshal(content, req); err != nil {
      return apperror.NewError(apperror.ErrTypeMalformedRequest, nil)
    }
    return nil

  default:
    return apperror.NewError(apperror.ErrTypeMethodNotSupported, nil)
  }
}
